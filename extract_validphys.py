...
for i in range(len(covmat_C)):
    C[i] = [float(c) for c in covmat_C.split('\t')[3:]]

covmat_CS = open("covmat/" + root + "/output/tables/groups_covmat.csv").readlines()[4:]
CS = np.zeros(shape=(len(covmat_CS), len(covmat_CS)))
for i in range(len(covmat_CS)):
    CS[i] = [float(c) for c in covmat_CS.split('\t')[3:]]

S = np.array(CS - C)

covmat_CSINV = open("covmat/" + root + "/output/tables/groups_invcovmat.csv").readlines()[4:]
CSINV = np.zeros(shape=(len(covmat_CSINV), len(covmat_CSINV)))
for i in range(len(covmat_CSINV)):
    CSINV[i] = [float(c) for c in covmat_CSINV.split('\t')[3:]]
...
theory_values = np.zeros(shape=(len(covmat_DT), len(covmat_DT[0].split('\t'[4:]))))
for i in range(len(D)):
    D[i] = covmat_DT[i].split('\t')[2]
    T[i] = covmat_DT[i].split('\t')[3]
    theory_values[i] = covmat_DT[i].split('\t')[4:]
